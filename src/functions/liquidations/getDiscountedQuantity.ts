import { SupportedTokensTickers, tokenData } from "../../ao/utils/tokenAddressData";
 ...
 */
function getTokenValue(from: { token: string, quantity: BigInt }, to: string, prices: RedstonePrices) {
  // token datas
  const fromData = {
    price: prices[from.token === "QAR" ? "AR" : from.token].v,
    scale: BigInt(10) ** tokenData[from.token as SupportedTokensTickers].denomination
  };
  const fromScaledPrice = BigInt(Math.round(fromData.price * Number(fromData.scale)));
  const toData = {
  ...
  const fromValUSD = from.quantity as bigint * fromScaledPrice / fromData.scale;
...
export function getDiscountedQuantity({
  ...
  priceData,
  validateMax = false
}: GetDiscountedQuantity, precisionFactor: number) {
  ...
  if (!dept || !reward) {
    throw new Error("Liquidated token or reward token is not in the user's position");
  }
  ...
  if (qualifyingPosition.discount as bigint > BigInt(0)) {
    ...
    marketValue = marketValue * (precise100 + (qualifyingPosition.discount as bigint)) / precise100;
  }
